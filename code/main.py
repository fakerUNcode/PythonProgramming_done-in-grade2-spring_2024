...
def set_seaborn_properties(context='talk', font_scale=0.8):
    sns.set_theme(context=context, font='Arial Unicode MS', font_scale=font_scale,
                  rc={'axes.unicode_minus': False,
                      'figure.figsize': (12, 8),
                      'figure.dpi': 150})


# 获取2020年的国家地区数据，并把它们封装成DataFrame。
...
# 分析和可视化全球每年的互联网用户总数以及全球每年每100人的移动端互联网订阅数、
# 互联网使用人数比例和每100人的宽带订阅数的平均值。
def global_internet_users_analysis():
    ...
    plt.subplot(2, 2, 1)
    plt.title('全球每年互联网用户总数', fontproperties='Arial Unicode MS')
    plt.xlabel('年份')
    plt.ylabel('全球每年互联网用户总数', fontproperties='Arial Unicode MS')
...
def entities_2020_internet_users_percentage_pie_bar():
    ...
    explode_arr[0] = 0.07
    plt.axes(aspect=1)
    plt.title('2020年各个国家地区的互联网用户占比', fontproperties='Arial Unicode MS')
...
def entities_2020_internet_users_percentage_distribution_histogram():
    ...
    plt.title('2020年各国家地区互联网用户占比分布直方图', fontproperties='Arial Unicode MS')
...
# 绘制2020年个国家地区互联网用户占比和移动互联网订阅量的散点图，并用线性回归分析这两个指标的关系。
def entities_2020_internet_users_percentage_distribution_scatter():
    set_seaborn_properties()
    entity_2020_df = get_2020_entities_dataframe()
    plt.title('2020年个国家地区互联网用户占比和移动互联网订阅量散点图', fontproperties='Arial Unicode MS')
    plt.xlabel('互联网用户占比占比')
    plt.ylabel('移动互联网订阅量')
...
def draw_internet_users_percentage_annual_top_3_wordcloud():
    ...
    wc.generate(text)
    plt.title('每年互联网用户的比例最大的国家地区名词云', fontproperties='Arial Unicode MS')
    plt.imshow(wc)
    plt.axis("off")
...
# 分析和可视化中国互联网用户的数据，包括各项指标的数值图、各项指标的增长率图以及对中国互联网到2050年的总用户数的预测。
def chinese_users_analysis():
    ...
    plt.savefig(
        '../img/中国互联网用户的数量（单位：千万人）、占人口的比例、移动互联网订阅每一百人比例、宽带每一百人订阅比例.png')
    plt.show()
    ...
    plt.legend(loc='upper left')
    plt.savefig(
        '../img/中国互联网用户的数量（单位：千万人）、占人口的比例、移动互联网订阅每一百人比例、宽带每一百人订阅比例的增长率.png')
    plt.show()

    # 利用多元线性回归预测中国互联网到2050年的总用户数
    # 拟合：
    set_seaborn_properties()
    plt.title('对1980到2020年中国互联网总用户数的拟合', fontproperties='Arial Unicode MS')
    ...
    # 预测：
    set_seaborn_properties()
    plt.title('到2030年中国互联网总用户数的预测', fontproperties='Arial Unicode MS')
    plt.xlabel('年份')
    plt.ylabel('人数（单位：千万人）')
